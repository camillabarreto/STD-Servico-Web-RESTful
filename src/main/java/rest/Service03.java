...
//                for(Replica replica : dataBase.getReplicas()){
//                    WebTarget t = client.target(UriBuilder.fromUri(replica.getEndpoint()).build());
//                    Response response = t.path(replica.getId()).path("operation").request(MediaType.TEXT_PLAIN + ";charset=utf-8").put(Entity.entity(operation.getId(),MediaType.APPLICATION_JSON), Response.class);
//                    System.out.println("Status de resposta: "+response.getStatus());
//                }
//                //realizar a operação
//                this.dataBase.addActions(operation.getId(), "success");
...
//                for(Replica replica : dataBase.getReplicas()){
//                    WebTarget t = client.target(UriBuilder.fromUri(replica.getEndpoint()).build());
//                    Response response = t.path(replica.getId()).path("operation").request(MediaType.TEXT_PLAIN + ";charset=utf-8").delete(); //delete(Entity.entity(operation,MediaType.APPLICATION_JSON), Response.class)
//                    System.out.println("Status de resposta: "+response.getStatus());
//                }
//                this.dataBase.addActions(operation.getId(), "fail");
//                return Response.status(403).build();
